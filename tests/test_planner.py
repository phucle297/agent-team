...
class TestPlanner:
    @patch("agents.planner.get_google")
    def test_returns_plan_from_llm(self, mock_get_google):
        mock_llm = MagicMock()
        mock_llm.invoke.return_value = FakeLLMResponse(
            "1. Design API (type: code)\n2. Research JWT (type: research)"
        )
        mock_get_google.return_value = mock_llm
    ...
    @patch("agents.planner.get_google")
    def test_passes_task_to_prompt(self, mock_get_google):
        mock_llm = MagicMock()
    ...
    @patch("agents.planner.get_google")
    def test_loads_prompt_template(self, mock_get_google):
        mock_llm = MagicMock()
        mock_llm.invoke.return_value = FakeLLMResponse("plan")
        mock_get_google.return_value = mock_llm
    ...
    @patch("agents.planner.get_google")
    def test_raises_on_missing_input(self, mock_get_google):
        from agents.planner import planner
        import pytest

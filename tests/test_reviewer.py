...
class TestReviewer:
    ...
    def test_approves_when_approved_in_response(self, mock_get_claude):
        mock_llm = MagicMock()
    ...
    @patch("agents.reviewer.get_claude")
    def test_rejects_when_needs_revision(self, mock_get_claude):
        mock_llm = MagicMock()
        mock_llm.invoke.return_value = FakeLLMResponse(
    ...
    @patch("agents.reviewer.get_claude")
    def test_increments_iteration(self, mock_get_claude):
        mock_llm = MagicMock()
    ...
    @patch("agents.reviewer.get_claude")
    def test_defaults_iteration_to_zero(self, mock_get_claude):
        mock_llm = MagicMock()
        mock_llm.invoke.return_value = FakeLLMResponse("## Status: APPROVED")
        mock_get_claude.return_value = mock_llm
    ...
    @patch("agents.reviewer.get_claude")
    def test_includes_code_and_plan_in_prompt(self, mock_get_claude):
        mock_llm = MagicMock()
    ...
    @patch("agents.reviewer.get_claude")
    def test_approved_only_when_before_needs_revision(self, mock_get_claude):
